...
const usePagination = ({
    count, page, onPageChange, siblingCount = 1, boundaryCount = 1, disabled
}: Props) => {
    ...
    const siblingStart = Math.max(Math.min(page + 1 - siblingCount),
        count - boundaryCount - siblingCount * 2 + 1)
    const siblingEnd = Math.min(Math.max(page + 1 + siblingCount, boundaryCount + siblingCount * 2 + 2), endPages.length > 0 ? endPages[0] - 2 : endPage - 1)
    console.log();
